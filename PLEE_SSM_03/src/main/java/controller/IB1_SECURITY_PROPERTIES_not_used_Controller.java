...
@Controller
public class IB1_SECURITY_PROPERTIES_not_used_Controller {
    //controller调service层
    @Autowired
    @Qualifier("IB1_SECURITY_PROPERTIESServiceImpl")
    private IB1_SECURITY_PROPERTIESService iB1_SECURITY_PROPERTIESService;

    @RequestMapping("/getIB1_SECURITY_PROPERTIES_not_used")
    public String getIB1_SECURITY_PROPERTIES(@RequestParam(value="platform_name", required=false) String platform_name, @RequestParam(value="IB1_VERSION", required=false) String IB1_VERSION,@RequestParam(value="diagnose_adr", required=false) String diagnose_adr,Model model){
        API api = new API();
        api.setStatus("true");
        ...
        try {
            ...
            result = iB1_SECURITY_PROPERTIESService.getIB1_SECURITY_PROPERTIES(map);
